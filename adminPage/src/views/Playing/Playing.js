...
export default function Playing() {
  ...
  const inactiveNumber = (no, level, number) => {
    return (
      <Button className={classes.actionButton + " " + classes.predictButton}>
        {no}
      </Button>
    );
  };
  ...
  const dataRead = async () => {
    try {
      const { data } = await authAxios.get("/enjoy-admin");
      if (data) {
        ...
        for (let i = 0; i < 1; i++) {
          ...
          for (let m = 0; m < 10; m++) {
            if (m % 5 === 0) {
              tmp_price = Math.floor(
                tmp_game_total[m] * 7.82 +
                  tmp_game_total[12] * 3.41 +
                  tmp_game_total[11 - (m % 2)] * 0.47
              );
              tmp_budget = 0;
              for (var k = 0; k < 13; k++) tmp_budget += tmp_game_total[k];
              tmp_budget = Math.floor(
                tmp_budget -
                  tmp_price -
                  tmp_game_total[m] -
                  tmp_game_total[12] -
                  tmp_game_total[11 - (m % 2)]
              );
            } else if (m % 2 === 0) {
              tmp_price = Math.floor(
                tmp_game_total[m] * 7.82 + tmp_game_total[11] * 0.96
              );
              tmp_budget = 0;
              for (k = 0; k < 13; k++) tmp_budget += tmp_game_total[k];
              tmp_budget = Math.floor(
                tmp_budget - tmp_price - tmp_game_total[m] - tmp_game_total[11]
              );
            } else {
              tmp_price = Math.floor(
                tmp_game_total[m] * 7.82 + tmp_game_total[10] * 0.96
              );
              tmp_budget = 0;
              for (k = 0; k < 13; k++) tmp_budget += tmp_game_total[k];
              tmp_budget =
                tmp_budget - tmp_price - tmp_game_total[m] - tmp_game_total[10];
            }
        ...
        <Card>
          ...
          <CardBody className={classes.customCardContentClass}>
            <div className={classes.innerContent}>
              <h5>
                Period : {period}{" "}
                <small>{Math.floor(timer / 60) + ":" + (timer % 60)}</small>
              </h5>
            </div>
            <div className={classes.innerContent}>
              <h5>DOGE</h5>
            ...
            <Table
              ...
              tableHead={[
                ...
                "8",
                "9",
                <span className={classes.greenItem}>G</span>,
                <span className={classes.redItem}>R</span>,
                <span className={classes.violetItem}>V</span>,
              ]}
