...
export default function Reviews(props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await authAxios.get(`/admin/reviews/` + page).catch(err => {
          catchFunc(err);
        });
        setList(data.reviews);
        setLast_page(parseInt(data.last));
      } catch (err) {
        catchFunc(err);
      }
  ...
  const askRemove = (id) => () => {
    setAlert(
      <SweetAlert
        ...
        title="Are you sure to remove this review data?"
        ...
        showCancel
      >
        You will not be able to recover this review data!
      </SweetAlert>
    );
  };
  const hideAlert = () => {
  ...
  const removeRecharge = async (id) => {
    try {
      const { data } = await authAxios.delete(`/admin/review/` + id).catch(err => {
        catchFunc(err);
      });
      setRefresh(!refresh);
      setAlert(
